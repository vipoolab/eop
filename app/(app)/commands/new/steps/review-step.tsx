...
export function ReviewStep({ state }: Props) {
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Letter preview — paper style for demo wow factor */}
      <section className="bg-slate-100 border border-slate-200 rounded-sm p-6">
        <div className="flex items-center gap-2 mb-3 text-xs text-slate-500">
          <FileText className="h-3.5 w-3.5" />
          ตัวอย่างหนังสือสั่งการ — รูปแบบราชการ
        </div>
        <div className="bg-white border border-slate-300 shadow-md max-w-3xl mx-auto p-10 font-[var(--font-thai)]">
          {/* Header */}
          ...
          {/* Doc number row */}
          <div className="flex justify-between mt-4 mb-6 text-sm text-slate-700">
            <div>
              <span className="text-slate-500">ที่ </span>
              <span className="font-mono">{"<auto-generated>"}</span>
            </div>
            <div>วันที่ออก: {new Date().toLocaleDateString("th-TH", { year: "numeric", month: "long", day: "numeric" })}</div>
          </div>
          ...
          <div className="space-y-1.5 mb-5">
            ...
            <div className="text-sm text-slate-900">{letter.recipient}</div>
            {letter.references && letter.references.length > 0 && (
              <div className="text-sm text-slate-900">
                <span className="text-slate-600">อ้างถึง </span>
                {letter.references.map((r, idx) => (
                  <span key={idx}>
                    {idx > 0 && " "}
          ...
          {/* Body */}
          <div className="space-y-3 text-sm text-slate-900 leading-loose">
            <p className="indent-12 text-justify">{letter.introduction}</p>
            <div className="space-y-1.5">
              {letter.directives.map((d, idx) => (
                <p key={idx} className="indent-12 text-justify">
                  {d}
                </p>
              ))}
            </div>
            {letter.reportInstruction && (
              <p className="indent-12 text-justify">{letter.reportInstruction}</p>
            )}
            <p className="indent-12 text-justify">{letter.closing}</p>
          </div>

          {/* Signature */}
          <div className="mt-12 text-right text-sm text-slate-900">
            <div>(ลงชื่อ) ........................................................</div>
            <div className="mt-1">(........................................)</div>
            <div className="mt-1 text-xs text-slate-500 italic">
              ลายเซ็นและชื่อผู้สั่งการ จะเซ็นและประทับตราเมื่อกดส่ง
            </div>
          </div>
        </div>
      </section>
